...
void send_data( zenith_sensor_handle_t sensor ) {
    ...
    ESP_ERROR_CHECK( zenith_sensor_read_temperature( sensor, &data_packet.sensor_data.temperature ) );
    ESP_ERROR_CHECK( zenith_sensor_read_humidity( sensor, &data_packet.sensor_data.humidity ) );
...
void app_main( void )
{
    ...
    ESP_ERROR_CHECK( init_sensor( &sensor ) );
    ...
    send_data( sensor ); // Send the data
